...
import { Users, MessageSquare, AlertTriangle, BookOpen } from 'lucide-react';

export const dynamic = 'force-dynamic';
export const metadata = { title: 'Admin — Overview' };

export default async function AdminOverviewPage() {
  const [userCount, feedbackCount, errorCount, examCount, recentFeedback, recentErrors] =
    await Promise.all([
      prisma.user.count(),
      ...
      prisma.errorLog.findMany({
        orderBy: { createdAt: 'desc' },
        take: 5,
        select: { id: true, errorType: true, errorMessage: true, email: true, createdAt: true },
      }),
    ]);
  ...
  const stats = [
    ...
    { label: 'Error logs',       value: errorCount,    icon: AlertTriangle, color: 'text-error' },
  ...
  return (
    <div className="space-y-8 max-w-5xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-surface border border-border rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-border">
            <h2 className="text-sm font-semibold text-light-text">Recent errors</h2>
          </div>
          <div className="divide-y divide-border">
            {recentErrors.length === 0 && (
              <p className="px-5 py-4 text-sm text-dark-text">No errors logged yet.</p>
            )}
            {recentErrors.map((e) => (
              <div key={e.id} className="px-5 py-3 flex items-start gap-3">
                <span className={`mt-0.5 text-[10px] font-medium px-1.5 py-0.5 rounded border shrink-0 ${
                  e.errorType === 'parse'
                ...
                <div className="min-w-0">
                  <p className="text-xs text-dark-text truncate">{e.email ?? 'Anonymous'}</p>
                  <p className="text-xs text-light-text/70 truncate mt-0.5">{e.errorMessage}</p>
                </div>
                <span className="ml-auto text-[10px] text-dark-text/60 shrink-0">
