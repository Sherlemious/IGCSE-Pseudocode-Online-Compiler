...
const TraceTable: React.FC<TraceTableProps> = ({ rows, maxRows }) => {
      ...
      <div className="h-full flex flex-col items-center justify-center gap-3 text-dark-text select-none p-4">
        ...
        <div className="text-center space-y-1">
          <div className="text-xs text-dark-text/50">
            Press <strong className="text-light-text">Trace</strong> to generate a dry-run table
          </div>
  ...
  return (
    <div className="p-3" style={{ fontSize: 'var(--editor-font-size)' }}>
      {/* Copy control */}
      ...
      <div className="overflow-x-auto scrollbar-thin scrollbar-thumb-primary scrollbar-track-background scrollbar-thumb-rounded-full">
        <table className="w-full text-xs font-mono border-collapse">
          <thead>
            <tr className="text-dark-text/60 border-b border-border">
              <th className="text-right py-1 pr-2 font-normal sticky left-0 bg-background">#</th>
              <th className="text-right py-1 px-2 font-normal">Line</th>
              {columns.map((name) => (
                <th key={name} className="text-left py-1 px-2 font-normal text-light-text/80 whitespace-nowrap">
                  {name}
                </th>
              ))}
              <th className="text-left py-1 px-2 font-normal">OUTPUT</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row, rowIdx) => {
              ...
              return (
                <tr key={row.step} className="border-b border-border/10 hover:bg-surface/40">
                  <td className="text-right py-1 pr-2 text-dark-text/40 sticky left-0 bg-background">{row.step}</td>
      ...
      {truncated && (
        <div className="mt-3 flex items-center gap-1.5 text-[10px] text-warning bg-warning/5 border border-warning/20 rounded px-2.5 py-1.5">
