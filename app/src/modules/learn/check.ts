...
export async function checkLessonCode(lesson: LearnLesson, code: string): Promise<LessonCheckResult> {
  if (lesson.type === 'quiz') {
    return { ok: false, message: 'This lesson is a quiz — pick the answers on the left.' };
  }

  const missing = lesson.mustContain ? containsAll(code, lesson.mustContain) : null;
  if (missing !== null) {
    return { ok: false, message: `Your code must include \`${missing}\`.` };
  }

  const forbidden = lesson.mustNotContain ? containsAny(code, lesson.mustNotContain) : null;
  if (forbidden !== null) {
    return { ok: false, message: `Remove \`${forbidden}\` from your code.` };
  }
  ...
  if (cases.length === 0) {
    return { ok: false, message: 'This lesson has nothing to check yet.' };
  }
  ...
  return { ok: true, message: cases.length > 1 ? `All ${cases.length} tests passed.` : 'Output matches.' };
}
