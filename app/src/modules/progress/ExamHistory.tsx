...
function ExamRowBody({ exam }: { exam: ExamItem }) {
...
function RowShell({ href, children }: { href?: string; children: ReactNode }) {
  const className = `flex items-center gap-3 py-2 border-b border-border/20 last:border-0 -mx-2 px-2 rounded-lg ${
    href ? 'hover:bg-background/50 transition-colors group' : ''
  }`;
  if (href) {
...
export default function ExamHistory({ exams, linkResults = true, showCta = true }: Props) {
  if (exams.length === 0) {
    return (
  ...
  return (
    <div className="bg-surface rounded-xl border border-border p-5 animate-fade-in-up" style={{ animationDelay: '300ms' }}>
      ...
      <div className="space-y-0.5">
        {exams.map((exam) => (
          <RowShell key={exam.id} href={linkResults ? `/exam/${exam.id}/results` : undefined}>
            <ExamRowBody exam={exam} />
          </RowShell>
        ))}
      </div>
    </div>
