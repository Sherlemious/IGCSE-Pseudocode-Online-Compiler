...
/** WhatsApp / Teams stamp — just the owl, large enough to read at 80px. */
export function OpenGraphMark() {
  const { ink, paper } = BRAND.colors;
  const mark = markDataUri({ color: paper, size: 1024 });
  return (
    <div
      style={{
        width: OG_SQUARE.width,
        height: OG_SQUARE.height,
        display: 'flex',
        alignItems: 'center',
    ...
    >
      <img src={mark} width={860} height={860} alt="" />
    </div>
  );
}

/** Wide exam-sheet card for Twitter / LinkedIn. */
